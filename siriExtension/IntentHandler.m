...
@interface IntentHandler () <INSendMessageIntentHandling, INStartAudioCallIntentHandling, INStartVideoCallIntentHandling>
...
- (void)confirmStartVideoCall:(INStartVideoCallIntent *)intent completion:(void (^)(INStartVideoCallIntentResponse * _Nonnull))completion {
    ...
    NSNumber *loggedIn = [defaults objectForKey:@"loggedIn"];
    if (!loggedIn || [loggedIn isEqualToNumber:@NO]) {
        INStartVideoCallIntentResponse *response = [[INStartVideoCallIntentResponse alloc] initWithCode:INStartVideoCallIntentResponseCodeFailureAppConfigurationRequired userActivity:nil];
        completion(response);
        return;
    }
    
    NSUserActivity *activity = [[NSUserActivity alloc] initWithActivityType:NSStringFromClass(INStartVideoCallIntent.class)];
    INStartVideoCallIntentResponse *response = [[INStartVideoCallIntentResponse alloc] initWithCode:INStartVideoCallIntentResponseCodeReady userActivity:activity];
    completion(response);
}

- (void)handleStartVideoCall:(nonnull INStartVideoCallIntent *)intent completion:(nonnull void (^)(INStartVideoCallIntentResponse * _Nonnull))completion {
    NSUserActivity *activity = [[NSUserActivity alloc] initWithActivityType:NSStringFromClass(INStartVideoCallIntent.class)];
    INStartVideoCallIntentResponse *response = [[INStartVideoCallIntentResponse alloc] initWithCode:INStartVideoCallIntentResponseCodeContinueInApp userActivity:activity];
    completion(response);
}
