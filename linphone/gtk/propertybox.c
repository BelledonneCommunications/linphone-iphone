...
void sound_section_fill(SoundSection *sec, LinphoneCore *lc)
{
	...
	{
			const gchar *ringfile=linphone_core_get_ring(lc);
			gtk_file_chooser_set_filename(GTK_FILE_CHOOSER(sec->ringfileentry),ringfile);
...
void sound_section_apply(SoundSection *sec, LinphoneCore *lc)
{
	...
	/* get ring path */
	tmp=gtk_file_chooser_get_filename(GTK_FILE_CHOOSER(sec->ringfileentry));
	if(tmp != NULL)
	{
                                        ...
                                        gpointer         user_data)
{
	...
	/* get ring path */
	tmp=gtk_file_chooser_get_filename(GTK_FILE_CHOOSER(prop->sound.ringfileentry));
...
void linphone_property_box_apply (gint pagenum)
{	
	LinphonePropertyBox *prop=get_property_box();
	LinphoneCore *lc=get_core();
	switch(pagenum)
	...
	gpointer         user_data)
{
	...
	switch(response_id)
	{
		...
		for(i = 0; i < 4; i++)
		{
			linphone_property_box_apply(i);
		}
		gtk_widget_destroy(GTK_WIDGET(dialog));
		break;
		case GTK_RESPONSE_APPLY:
		{
			GtkNotebook *notebook = GTK_NOTEBOOK(lookup_widget(get_property_box()->prop,"prop1notebook"));
