...
class ProviderDelegate: NSObject {
	...
	func reportIncomingCall(call:Call?, uuid: UUID, handle: String, hasVideo: Bool) {
		...
		provider.reportNewIncomingCall(with: uuid, update: update) { error in
			if error == nil {
				for proxyConfig in CallManager.instance().lc!.proxyConfigList {
					if proxyConfig.state == .Progress || proxyConfig.state == .Ok {
						CallManager.instance().providerDelegate.endCallNotExist(uuid: uuid, timeout: .now() + 20)
						return
					}
				}
				// registry failed or cleared, need to terminate the callkit immediately
