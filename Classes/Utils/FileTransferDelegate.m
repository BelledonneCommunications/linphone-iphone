...
static void linphone_iphone_file_transfer_recv(LinphoneChatMessage *message, const LinphoneContent *content,
	...
	if (size == 0) {
        ...
        if ([fileType isEqualToString:@"image"]) {
            ...
            NSString *key = [fileType isEqualToString:@"mkv"] ? @"localsound" : ([fileType isEqualToString:@"file"] ? @"localfile" : @"localvideo");
...
- (void)uploadFile:(NSData *)data forChatRoom:(LinphoneChatRoom *)chatRoom withUrl:(NSURL *)url {
    ...
    if ([[url pathExtension] isEqualToString:@"MOV"])
        ...
        [self uploadData:data forChatRoom:chatRoom type:@"mkv" subtype:nil name:name key:@"localsound" keyData:name qualityData:nil];
    else
