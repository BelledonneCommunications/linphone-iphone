		...
		switch cstate {
			case .IncomingReceived:
				if (CallManager.callKitEnabled()) {
					...
					} else if (!(ConfigManager.instance().lpConfigBoolForKey(key: "account_pushnotification")) && CallManager.instance().alreadyRegisteredForNotification && UIApplication.shared.isRegisteredForRemoteNotifications) {
